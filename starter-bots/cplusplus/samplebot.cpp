	...
	void readinText()
	{
		...
		while ( getline(statefile,inputline) )
		{
			...
			if(inputline == "XXXXXXXXX GAME INFO XXXXXXXXX") 
			{
				...
				roundNumber = stoi(inputline.substr(inputline.find(":") + 1));
				
				getline(statefile,inputline);
				
				getline(statefile,inputline);
				mapWidth = stoi(inputline.substr(inputline.find(":") + 1));
				
				getline(statefile,inputline);
				mapHeight = stoi(inputline.substr(inputline.find(":") + 1));

				getline(statefile,inputline);
				income = stoi(inputline.substr(inputline.find(":") + 1));
			}
			
			//start of building info input
			if(inputline == "****** BUILDING STATS ******")
			{
				getline(statefile,inputline);
				getline(statefile,inputline); //skip prototype
				...
				while(inputline != "*****************************")
				{
					...
					int building_energyCost = stoi(inputline);
